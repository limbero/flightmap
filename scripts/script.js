...
function newFlight(between, places) {
  trips.push({
    between: between,
    polygon: newPath(
      [places[between[0]].coords, places[between[1]].coords],
      '#FF6666'
    ),
    type: 'flight',
  });
...
function newPlace(location, id, places, layover) {
  ...
  google.maps.event.addListener(marker, 'mouseover', function(event) {
    if (places[this.id].layover) {
      return;
    }
    for (let id in place_markers) {
      place_markers[id].setIcon(
        circleIcon(0.1, iconSizeAtZoomLevel[map.getZoom()])
      );
    }
    ...
    for (let i = 0; i < trips.length; i++) {
      if (this.id == trips[i].between[0]) {
        place_markers[trips[i].between[1]].setIcon(
          circleIcon(
            places[trips[i].between[1]].layover ? 0.0 : 1.0,
            iconSizeAtZoomLevel[map.getZoom()]
          )
        );
        ...
        place_markers[trips[i].between[0]].setIcon(
          circleIcon(
            places[trips[i].between[0]].layover ? 0.0 : 1.0,
            iconSizeAtZoomLevel[map.getZoom()]
          )
        );
...
function initMap() {
  ...
  google.maps.event.addListener(map, 'zoom_changed', function() {
    ...
    for (let id in place_markers) {
      ...
      place_markers[id].setIcon(
        circleIcon(oldOpacity, iconSizeAtZoomLevel[map.getZoom()])
      );
    }
  });
  ...
  fetch('data/places.json')
    ...
    .then(places => {
      ...
      fetch('data/trainrides.json')
        .then(response => response.json())
        .then(trainrides => {
          trainrides.map(trainride =>
            newTrainRide(trainride.between, trainride.coords)
          );
          ...
          fetch('data/busrides.json')
            ...
            .then(busrides => {
              busrides.map(busride =>
                newBusRide(busride.between, busride.coords)
              );
              ...
              fetch('data/drives.json')
                ...
                .then(drives => {
                  ...
                  fetch('data/flights.json')
                    .then(response => response.json())
                    .then(paths =>
                      paths.map(between => newFlight(between, places))
                    );
                });
            });
        });
